...
\begin{frame}[label=current]{Geographic normalization as text to polygon}
\begin{columns}
\begin{column}{.45\textwidth}
\begin{tikzpicture}[font=\footnotesize]
\pgfmathsetmacro{\height}{.7}
\pgfmathsetmacro{\width}{1.5}
\node[text width=.5\linewidth, align=left, anchor=south] (text-in) at (-\width, 0) {between the towns of \textcolor{ua-oasis}{Adrano} and \textcolor{ua-oasis}{S. Maria di Licodia}, 32 kilometers (20 mi) northwest of \textcolor{ua-oasis}{Catania}};
\node[anchor=south] (map-in) at (\width, 0) {\includegraphics[width=.3\linewidth]{compgeo/biancavilla.png}};
\node[draw, fill=black!25, align=center, inner sep=5pt] (model) at (0, -\height) {Generative Language Model}
edge[latex-] (text-in)
edge[latex-] (map-in);
\node[align=left, anchor=north, inner sep=0pt] (code) at (0, -2*\height) {%
...
edge[latex-] (model);
\node[anchor=north] (map-out) at (0, -4*\height) {\includegraphics[width=.3\linewidth]{compgeo/biancavilla.png}}
edge[latex-] (code);
\end{tikzpicture}
\end{column}
\begin{column}{.45\textwidth}
