...
\begin{frame}[t]{Concept normalization: task definition}
\centering
...
\node[ua-oasis] at (4*\width, \height) {\large Prediction};
...
\node[model] (reranker) at (3*\width, -1.5*\height) {Machine \\ Learning \\ Reranker};
\foreach[count=\i from 0, count=\slide from 4] \candidate/\prediction in {
  ...
  \node[ua-oasis, align=center] (candidate-\i) at (2*\width, -\height*\i) {\candidate} edge[latex-] (umls) edge[-latex] (reranker);
  \node[ua-oasis, align=center] (prediction-\i) at (4*\width, -\height*\i) {\prediction} edge[latex-] (reranker);
}
\end{tikzpicture}
\end{frame}
...
\foreach \i/\name/\textcolor/\phrase in {
    -4/p/black/heart\\attack,
    0/i/ua-red/myocardial\\infarction,
    4/n/black/cardiovascular\\infections} {
...
\begin{frame}{Concept normalization evaluation}{\subtitlecite{xu-bethard-2021-triplet}}
Evaluation data:
