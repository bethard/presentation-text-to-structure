...
\pause
\node[draw] (calendar1) at (chatgpt) {\includegraphics[width=.5\textwidth]{timeline/washington-president.png}};
\node[draw, anchor=north] (calendar2) at (calendar1.south) {\includegraphics[width=.5\textwidth]{timeline/washington-chancellor.png}};
\end{tikzpicture}
\end{frame}
...
\section{Lesson 2: Structured Prompt $>$ Graph-Structured Network}
...
\subsection{Introduction to timeline-based question answering}

\subsection{Linearizing timelines into question answering input}

\section{Lesson 3: External Structure Execution $>$ Implicit Network Reasoning}
...
\subsection{Predicting geographic attributes for executable constraints}
...
\node[anchor=south] (map-in) at (\width, -.7*\height) {\includegraphics[width=.3\linewidth]{compgeo/map-biancavilla-in.png}};
